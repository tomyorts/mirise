...
export const REVIEW: Record<string, ArticleReview> = {
  ...
  // 抜歯矯正と非抜歯矯正の判断基準
  'basshi-hibassi': {
    publish: true, // 2026-09-15 監修確定
    values: {
      cost: '検査・診断料 数万円、装置・技術料 60万円〜150万円、調整料 月数千円〜1万円程度',
    },
  },
  ...
  'geka-nagare': {
    publish: false,
    values: {
      hokenGaku: '◯◯万円程度', // 保険適用(3割負担)の自己負担 総額の目安
      jihiGaku: '◯◯万円程度', // 保険適用外(自由診療)の総額の目安
      cost: '保険適用(3割負担)の場合、矯正治療と手術・入院を合わせた総額◯◯万円程度。高額療養費制度の対象となる場合があります。保険適用の条件を満たさない場合は自由診療となり総額◯◯万円程度',
    },
  },
  ...
  saichiryou: {
    publish: false,
    values: {
      honbunHiyou: '前歯部だけの部分矯正なら◯◯万円台から、診断からやり直す全体矯正なら◯◯万円前後が目安です。',
      cost: '部分矯正 ◯◯万円〜、全体矯正 ◯◯万円〜◯◯万円。精密検査・診断料、保定装置代が別途かかる場合があります。',
    },
  },
  ...
  // 顎間固定中の生活:期間はどのくらい?食事・会話・睡眠・仕事はどうなるか
  'gakukan-kotei': {
    publish: false,
    values: {
      cost: '顎変形症の診断のもと、指定を受けた医療機関で手術を前提とした治療を行う場合は健康保険が適用され、自己負担3割で矯正治療と手術を合わせて総額◯◯万円程度が目安です。高額療養費制度の対象になる場合があります。',
    },
  },

  // 顎変形症の手術後、顔の腫れはいつまで続く?経過の目安と過ごし方
  'hare-fukki': {
    publish: false,
    values: {
      cost: '顎変形症の診断のもと、指定を受けた医療機関で手術を前提とした治療を行う場合は健康保険が適用され、自己負担3割で矯正治療と手術を合わせて総額◯◯万円程度が目安です。高額療養費制度の対象になる場合があります。',
  ...
  // 下顎後退(顎が小さい・引っこんでいる)の治療|矯正でできることと手術が検討される場合
  'kagaku-koutai': {
    publish: true, // 2026-09-15 監修確定
    values: {
      cost: '顎変形症の診断のもと、指定を受けた医療機関で手術を前提とした治療を行う場合は健康保険が適用され、自己負担3割で矯正治療と手術を合わせて総額60万〜80万円程度が目安です。保険適用の条件を満たさない場合は自由診療となります。',
    },
  },

  // 顎の手術で入れたプレートは抜くべき?除去手術の時期・方法と抜かない場合の経過
  'plate-jokyo': {
    publish: false,
    values: {
      cost: '顎変形症の一連の治療として保険適用の対象になる場合、自己負担3割でのプレート除去手術の費用は入院の有無や麻酔方法により変わります。一連の外科矯正全体の自己負担は総額◯◯万円程度が目安です。',
    },
  },

  // 外科矯正と親知らず:手術前に抜く理由と、いつ・どこで抜くかの考え方
  'oyashirazu-geka': {
    publish: false,
    values: {
      cost: '親知らずの抜歯は多くの場合保険診療で行われます。外科矯正全体では、顎変形症の診断のもと指定医療機関で治療する場合に健康保険が適用され、自己負担3割で総額◯◯万円程度が目安です。',
    },
  },
};
